...
def _send_via_smtp(to_address: str, subject: str, html_content: str, plain_content: str = None, attachments: list = None):
    ...
    if not all([smtp_server, smtp_port_str, smtp_username, smtp_password, from_address]):
        raise RuntimeError("SMTP configuration (SERVER, PORT, USERNAME, PASSWORD, FROM) is incomplete.")
...
def _send_via_sendgrid(to_address: str, subject: str, html_content: str, plain_content: str = None, attachments: list = None):
    """Send an email via SendGrid API using the SendGrid Python client."""
    api_key = os.getenv('SENDGRID_API_KEY')
    from_email = os.getenv('SENDGRID_FROM_EMAIL')
    if not api_key or not from_email:
        raise RuntimeError("SendGrid API key or sender email is not configured (SENDGRID_API_KEY, SENDGRID_FROM_EMAIL).")
    ...
    if attachments:
        for attachment_data in attachments:
            ...
            content_b64 = attachment_data.get("content")
            content_type = attachment_data.get("type", "application/octet-stream")
            disposition = attachment_data.get("disposition", "attachment") # 'attachment' or 'inline'
...
def send_email(to_address: str, subject: str, html_content: str, plain_content: str = None, attachments: list = None):
    """
    ...
    fallback_method = _send_via_smtp if use_sendgrid else _send_via_sendgrid # Fallback is the other one
    ...
    try:
        ...
        print(f"⚠️ Email send failed via primary method ({'SendGrid' if use_sendgrid else 'SMTP'}): {e}")
        ...
        try:
             ...
             if primary_method == _send_via_sendgrid: # Trying SMTP as fallback
                 if all([os.getenv('SMTP_SERVER'), os.getenv('SMTP_PORT'), os.getenv('SMTP_USERNAME'), os.getenv('SMTP_PASSWORD'), os.getenv('SMTP_FROM')]):
                     can_fallback = True
                 else:
                     print("Fallback to SMTP skipped: SMTP configuration incomplete.")
             else: # Trying SendGrid as fallback
                 if all([os.getenv('SENDGRID_API_KEY'), os.getenv('SENDGRID_FROM_EMAIL')]):
                     can_fallback = True
                 else:
                     print("Fallback to SendGrid skipped: SendGrid configuration incomplete.")

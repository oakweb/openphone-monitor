...
def send_openphone_sms(recipient_phone, message_body):
    ...
    try:
        ...
        except: try: log_func.error(f"OpenPhone API Error Response: Status={http_err.response.status_code}, Body={http_err.response.text}") except: pass
        return False
...
def index():
    ...
    try:
        db.session.execute(text("SELECT 1")); db_status = "Connected"; now_utc = datetime.now(timezone.utc); start_today_utc = datetime.combine(now_utc.date(), datetime.min.time(), tzinfo=timezone.utc); start_week_utc  = start_today_utc - timedelta(days=start_today_utc.weekday())
        count_today = (db.session.query(func.count(Message.id)).filter(Message.timestamp >= start_today_utc).scalar() or 0); count_week = (db.session.query(func.count(Message.id)).filter(Message.timestamp >= start_week_utc).scalar() or 0)
...
def properties_list_view():
    properties = Property.query.order_by(Property.name).all(); return render_template('properties_list.html', properties=properties)
...
def property_detail_view(property_id):
    ...
    try:
        ...
        current_app.logger.debug(f" Found {len(tenants)} tenants for property {prop.id}"); property_identifier_string = f'(ID:{property_id})'
        relevant_history = NotificationHistory.query.filter(NotificationHistory.properties_targeted.like(f'%{property_identifier_string}%')).order_by(NotificationHistory.timestamp.desc()).limit(50).all()
...
def messages_view():
     ...
     try:
        if target_phone_number:
            app.logger.debug(f"--- Loading DETAIL view for {target_phone_number} ---"); msgs_for_number = Message.query.options(joinedload(Message.property), joinedload(Message.contact)).filter(Message.phone_number==target_phone_number).order_by(Message.timestamp.desc()).all()
            app.logger.debug(f"Query for {target_phone_number} returned {len(msgs_for_number)} message objects."); is_known = False; contact_name = None
            ...
            app.logger.debug("--- Loading OVERVIEW view ---"); msgs_overview = Message.query.options(joinedload(Message.property), joinedload(Message.contact)).order_by(Message.timestamp.desc()).limit(100).all()
            app.logger.debug(f"Query overview returned {len(msgs_overview)} messages."); properties_list = Property.query.order_by(Property.name).all(); return render_template("messages_overview.html", messages=msgs_overview, properties=properties_list)
...
def assign_property():
    message_id_str = request.form.get('message_id'); property_id_str = request.form.get('property_id'); redirect_url = request.referrer or url_for('index')
    current_app.logger.info(f"Received assignment request via POST: message_id='{message_id_str}', property_id='{property_id_str}', referrer='{request.referrer}'"); message_id = None
...
def contacts_view():
    ...
    try:
        ...
        recent_distinct_numbers_result = db.session.execute(latest_distinct_numbers_query).all(); recent_distinct_numbers = [num for (num,) in recent_distinct_numbers_result]; count = 0; processed_keys = set()
...
def notifications_view():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            current_app.logger.info(f"Fetching current tenants for properties: {target_property_ids}"); target_tenants = Tenant.query.filter(Tenant.property_id.in_(target_property_ids), Tenant.status == 'current').all()
            ...
            if 'email' in channels and emails_to_send:
                ...
                for email in emails_to_send:
                    # *** CORRECTED try/except block for individual email send ***
                    try:
            ...
            total_email_attempts = len(emails_to_send) if 'Email' in channels_attempted else 0; total_sms_attempts = len(phones_to_send) if 'SMS' in channels_attempted else 0; total_successes = email_success_count + sms_success_count; total_attempts = total_email_attempts + total_sms_attempts
            ...
            if msg.local_media_paths and isinstance(msg.local_media_paths, str):
                ...
                for idx, path in enumerate(paths):
                    ...
                    if trimmed_path and is_image:
                        current_app.logger.debug(f"  >>> Adding image item: {trimmed_path}"); sender_name = msg.phone_number
                ...
                for idx, path in enumerate(msg.local_media_paths):
                     ...
                     if trimmed_path and is_image:
                         current_app.logger.debug(f"  >>> Adding image item from list: {trimmed_path}"); sender_name = msg.phone_number
                         if msg.contact and msg.contact.contact_name: sender_name = msg.contact.contact_name
                         image_items.append({"path": trimmed_path, "message_id": msg.id, "index": idx, "timestamp": msg.timestamp, "sender_info": sender_name})

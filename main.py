...
import werkzeug # Keep if abort uses it, otherwise can remove if only serve_upload used it
...
# Serve files from /app/static/ directory under the /static/ URL prefix
# Should handle subdirectories like /app/static/uploads automatically
# Use '/app/static' directly assuming standard container structure
...
# Flask's static_folder is still relevant for url_for('static', ...)
...
def send_openphone_sms(recipient_phone, message_body):
    ...
    payload = { "from": sending_number, "to": [recipient_phone], "content": message_body,}
    try:
...
def initialize_database(app_context):
    ...
    with app_context:
        ...
        try:
            ...
            try:
                db.session.execute(text("ALTER TABLE messages ADD COLUMN sid VARCHAR")); db.session.commit(); app.logger.info("✅ Ensured messages.sid column exists.")
...
def index():
    ...
    try:
        ...
        now_utc = datetime.now(timezone.utc); start_today_utc = datetime.combine(now_utc.date(), datetime.min.time(), tzinfo=timezone.utc)
...
def property_detail_view(property_id):
    ...
    try:
        ...
        ).order_by(NotificationHistory.timestamp.desc()).limit(50).all()
        current_app.logger.debug(f" Found {len(relevant_history)} relevant history entries for property {prop.id}")
        return render_template('property_detail.html', prop=prop, tenants=tenants, history=relevant_history)
    except Exception as e:
...
def messages_view():
     ...
     try:
        if target_phone_number:
            ...
            msgs_for_number = Message.query.options(joinedload(Message.property), joinedload(Message.contact)).filter(Message.phone_number==target_phone_number).order_by(Message.timestamp.desc()).all()
            app.logger.debug(f"Query for {target_phone_number} returned {len(msgs_for_number)} message objects.")
            is_known = False; contact_name = None
            ...
            msgs_overview = Message.query.options(joinedload(Message.property), joinedload(Message.contact)).order_by(Message.timestamp.desc()).limit(100).all()
            ...
            return render_template("messages_overview.html", messages=msgs_overview, properties=properties_list)
...
# (Keep assign_property as is - it's used elsewhere)
@app.route("/assign_property", methods=["POST"])
def assign_property():
    message_id_str = request.form.get('message_id'); property_id_str = request.form.get('property_id')
    ...
    if property_id_str and property_id_str.isdigit() and int(property_id_str) > 0:
        target_property_id = int(property_id_str); property_obj = db.session.get(Property, target_property_id)
        ...
        target_property_id = None; current_app.logger.info(f"Request to unassign property from message {message_id}")
    ...
    try:
        message.property_id = target_property_id; db.session.commit()
        ...
        db.session.rollback(); flash(f'Database error assigning property: {str(e)}', 'danger')
...
def contacts_view():
    ...
    if request.method == "POST":
        ...
        try:
            if action == "add":
                name = request.form.get("name", "").strip(); phone_key = request.form.get("phone", "").strip()
                if name and phone_key:
                    if len(phone_key) != 10 or not phone_key.isdigit(): flash("Invalid phone key format (10 digits).", "error")
                        ...
                        if existing:
                            is_default_name = (existing.contact_name.startswith('+') and len(existing.contact_name) > 1 and existing.contact_name[1:].isdigit()) or (len(existing.contact_name) == 10 and existing.contact_name.isdigit() and existing.contact_name == existing.phone_number)
                ...
                if not contact_key_to_delete: flash("No Contact ID provided.", "error")
                    ...
                    if not contact_to_delete: flash("Contact not found.", "error")
                        ...
                        if not dummy_contact: current_app.logger.error(f"CRITICAL: Dummy contact '{dummy_phone_key}' missing."); flash("Internal error: Default reference missing.", "danger"); contact_to_delete = None
                            ...
                            update_stmt = Message.__table__.update().where(Message.phone_number == contact_key_to_delete).values(phone_number = dummy_phone_key)
                            result = db.session.execute(update_stmt); updated_count = result.rowcount; current_app.logger.info(f"Re-assigned {updated_count} messages from contact {contact_key_to_delete} to dummy contact.")
                            current_app.logger.info(f"Marking Contact '{contact_to_delete.contact_name}' ({contact_key_to_delete}) for deletion.")
                            db.session.delete(contact_to_delete); flash(f"Contact '{contact_to_delete.contact_name}' deleted (Messages reassigned).", "success")
    ...
    try:
        ...
        message_alias = Message.__table__.alias('m'); row_num_col = func.row_number().over(partition_by=message_alias.c.phone_number, order_by=message_alias.c.timestamp.desc()).label('rn')
        ranked_messages_subq = select(message_alias.c.phone_number, message_alias.c.timestamp, row_num_col).select_from(message_alias).subquery('ranked_messages')
        latest_distinct_numbers_query = select(ranked_messages_subq.c.phone_number).where(ranked_messages_subq.c.rn == 1).order_by(ranked_messages_subq.c.timestamp.desc()).limit(50)
...
def update_contact_name():
    ...
    try:
        ...
        if contact_to_update:
            is_reverting = ( (new_name.startswith('+') and len(new_name) > 1 and new_name[1:].isdigit()) or (len(new_name) == 10 and new_name.isdigit()) )
    ...
    if request.method == "POST":
        ...
        for file in uploaded_files:
            if file and file.filename:
                filename = secure_filename(file.filename); file_content_bytes = file.read()
                content_type = file.content_type or mimetypes.guess_type(filename)[0] or 'application/octet-stream'
                attachments_data.append({'content_bytes': file_content_bytes, 'filename': filename, 'type': content_type})
        ...
        try:
            ...
            if not emails_to_send and not phones_to_send:
                ...
                db.session.add(history_log); db.session.commit(); current_app.logger.warning(f"Notification attempt logged (ID: {history_log.id}), but no recipients found."); return redirect(url_for('notifications_view'))
            ...
            current_app.logger.warning(f"Status Calc: Channels Attempted={channels_attempted}"); current_app.logger.warning(f"Status Calc: Email Success={email_success_count}, Email Attempts={total_email_attempts}"); current_app.logger.warning(f"Status Calc: SMS Success={sms_success_count}, SMS Attempts={total_sms_attempts}"); current_app.logger.warning(f"Status Calc: Total Successes={total_successes}, Total Attempts={total_attempts}")
...
def galleries_overview():
    ...
    try:
        property_ids_with_media = db.session.query(Message.property_id).filter(Message.property_id.isnot(None), Message.local_media_paths.isnot(None)).distinct().all()
        prop_ids = [pid for (pid,) in property_ids_with_media]; app.logger.debug(f"Found {len(prop_ids)} property IDs with media: {prop_ids}")
        ...
        unsorted_message_count_query = db.session.query(func.count(Message.id)).filter(Message.property_id.is_(None), Message.local_media_paths.isnot(None))
        unsorted_count = unsorted_message_count_query.scalar() or 0; app.logger.debug(f"Found {unsorted_count} unsorted messages with media.")
...
# (Keep gallery_for_property as is, using string splitting and correct sender_info)
@app.route("/gallery/<int:property_id>")
def gallery_for_property(property_id):
    prop = db.session.get(Property, property_id)
    if not prop: abort(404, description="Property not found")
    gallery_title = f"Gallery for {prop.name}"; image_items = []; error_message = None
    image_extensions = ('.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp', '.heic')
    try:
        messages_with_media = Message.query.options(joinedload(Message.contact)).filter(Message.property_id == property_id, Message.local_media_paths.isnot(None)).order_by(Message.timestamp.desc()).all()
        ...
        for msg in messages_with_media:
            if msg.local_media_paths and isinstance(msg.local_media_paths, str):
                paths = [p.strip() for p in msg.local_media_paths.split(',') if p.strip()]; current_app.logger.debug(f"  Paths split from string: {paths} (Msg ID: {msg.id})")
                for idx, path in enumerate(paths):
                    trimmed_path = path; current_app.logger.debug(f"  Checking path for gallery: '{trimmed_path}' (Msg ID: {msg.id})")
                    is_image = trimmed_path.lower().endswith(image_extensions) if trimmed_path else False; current_app.logger.debug(f"  Path ends with valid extension? {is_image}")
                ...
                for idx, path in enumerate(msg.local_media_paths):
                     trimmed_path = path.strip(); current_app.logger.debug(f"  Checking list path for gallery: '{trimmed_path}' (Msg ID: {msg.id})")
                     is_image = trimmed_path.lower().endswith(image_extensions) if trimmed_path else False; current_app.logger.debug(f"  List Path ends with valid extension? {is_image}")
                     if trimmed_path and is_image:
...
# (Keep unsorted_gallery as is, using string splitting)
@app.route('/unsorted')
def unsorted_gallery():
    current_app.logger.info("Accessing /unsorted route"); error_message = None; unsorted_items_for_template = []; properties_list = []
    image_extensions = ('.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp', '.heic')
    try:
        unsorted_messages = Message.query.options(joinedload(Message.contact)).filter(Message.property_id.is_(None), Message.local_media_paths.isnot(None)).order_by(Message.timestamp.desc()).all()
        ...
        for message in unsorted_messages:
            if message.local_media_paths and isinstance(message.local_media_paths, str):
                paths = [p.strip() for p in message.local_media_paths.split(',') if p.strip()]; current_app.logger.debug(f"  Paths split from string: {paths} (Msg ID: {message.id})")
                for path in paths:
                    trimmed_path = path; current_app.logger.debug(f"  Checking path for unsorted: '{trimmed_path}' (Msg ID: {message.id})")
                    is_image = trimmed_path.lower().endswith(image_extensions) if trimmed_path else False; current_app.logger.debug(f"  Path ends with valid extension? {is_image}")
                ...
                for path in message.local_media_paths:
                     trimmed_path = path.strip(); current_app.logger.debug(f"  Checking list path for unsorted: '{trimmed_path}' (Msg ID: {message.id})")
                     is_image = trimmed_path.lower().endswith(image_extensions) if trimmed_path else False; current_app.logger.debug(f"  List Path ends with valid extension? {is_image}")
        ...
        properties_list = Property.query.order_by(Property.name).all(); current_app.logger.info(f"Found {len(properties_list)} properties for the dropdown.")
...
def gallery_static():
    return render_template("gallery.html", image_items=[], gallery_title="Static Uploads Gallery")
...
def list_uploads():
     ...
     try: files = [f for f in os.listdir(upload_path) if os.path.isfile(os.path.join(upload_path, f))]; return jsonify({"uploads": files})
     except Exception as e: app.logger.error(f"Error listing uploads in {upload_path}: {e}", exc_info=True); return jsonify({"error": "Failed to list uploads", "details": str(e)}), 500
...
def clear_contacts_debug():
     ...
     try:
         ...
         update_stmt = Message.__table__.update().where(Message.phone_number != dummy_phone_key).values(phone_number = dummy_phone_key)
         result = db.session.execute(update_stmt); updated_count = result.rowcount; current_app.logger.info(f"Re-assigned {updated_count} messages to dummy contact ({dummy_phone_key}).")
         delete_stmt = Contact.__table__.delete().where(Contact.phone_number != dummy_phone_key)
         result = db.session.execute(delete_stmt); deleted_count = result.rowcount; current_app.logger.info(f"Deleted {deleted_count} non-dummy contacts.")
...
if __name__ == "__main__":
    ...
    # Use Waitress or another production server instead of app.run for production

...
from models import (
    Contact,
    Message,
)  # Message model must include local_media_paths column
...
def webhook():
    ...
    try:
        ...
        urls = [
            m.get("url") for m in media_items if isinstance(m, dict) and m.get("url")
        ]
        ...
        if not contact:
            ...
            try:
                db.session.commit()
                print(
                    f"✅ Contact created: {contact.contact_name} ({contact.phone_number})"
                )
            except Exception as e:
                db.session.rollback()
            ...
            if urls:
                ...
                for idx, url in enumerate(urls):
                    try:
                        ...
                        base = (
                            os.path.splitext(os.path.basename(parsed))[0]
                            or f"file{idx+1}"
                        )
                        safe = (
                            "".join(c for c in base if c.isalnum() or c in ["_", "-"])
                            or f"file{idx+1}"
                        )
